...
const Categories = () => {
    ...
    const changePage = (page)=>{
        setCurrentPage(page);
        navigate('/data/categories?page='+page);
    }


    useEffect(()=>{
        const pageTimeout = setTimeout(()=>{
            axios.post(process.env.REACT_APP_URL_API_DATA+'/categories/no-of-pages?size='+sizeOfPages,{search:search}).then((res)=>{
                if(res.data.no_of_pages === 0){
                    setNoRecord(true);
                    if(searchParams.get('page')){
                        setNoOfPages(0)
                        navigate('/data/categories?page=0');
                    }
                }else{
                    setNoRecord(false);
                    ...
                    if(currentPage>res.data.no_of_pages){
                        ...
                        navigate('/data/categories?page='+res.data.no_of_pages);
                    }
                }
            })
        },DELAY_TIME)
    ...
    useEffect(()=>{
        const dataTimeout = setTimeout(()=>{
            if(!noRecord){
                axios.post(process.env.REACT_APP_URL_API_DATA+'/categories/get?page='+currentPage+'&size='+sizeOfPages,{search:search}).then((res)=>{
                    setData(res.data)
                })
            }
        },DELAY_TIME)
        return ()=>{
            ...
            if(currentSearch.current !== search){
                setCurrentPage(1);
                navigate('/data/categories?page=1')
            }
        }
    ...
    const edit =(id)=>{
        navigate(`/data/categories/edit/${id}`,{state:{page:currentPage}})
    }

    const remove =(id)=>{
        axios.delete(`${process.env.REACT_APP_URL_API_DATA}/categories/delete/${id}`,{withCredentials:true}).then((res)=>{
            toggle(prev=>!prev);
        })
    }
