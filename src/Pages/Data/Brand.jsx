...
const Brand = () => {
    ...
    const changePage = (page)=>{
        setCurrentPage(page);
        navigate('/data/brand?page='+page+"&category="+Object.keys(isCategory)[0].split('_')[1]);
    }


    useEffect(()=>{
        const pageTimeout = setTimeout(()=>{
            axios.post(process.env.REACT_APP_URL_API_DATA+'/brand/no-of-pages?size='+sizeOfPages,{...isCategory,search:search}).then((res)=>{
                if(res.data.no_of_pages === 0){
                    setNoRecord(true);
                    if(searchParams.get('page')){   
                        setNoOfPages(0)
                        navigate('/data/brand?page=0&category='+Object.keys(isCategory)[0].split('_')[1]);
                    }
                }else{
                    setNoRecord(false);
                    ...
                    if(currentPage>res.data.no_of_pages){
                        ...
                        navigate('/data/brand?page='+res.data.no_of_pages+"&category="+Object.keys(isCategory)[0].split('_')[1]);
                    }
                    if(searchParams.get('page')=== '0'){
                        setCurrentPage(1)
                        navigate('/data/brand?page=1');
                    }
                }
            })
        },DELAY_TIME)
    ...
    },[sizeOfPages,data,navigate,searchParams,isCategory,currentPage,search])

    useEffect(()=>{
        const dataTimeout = setTimeout(()=>{
            if(!noRecord){
                axios.post(process.env.REACT_APP_URL_API_DATA+'/brand/get?page='+currentPage+'&size='+sizeOfPages,{...isCategory,search:search}).then((res)=>{
                    setData(res.data)
                })
            }
        },DELAY_TIME)
    ...
    },[isCategory,currentPage,dataRefresh,sizeOfPages,search,navigate,noRecord])
    ...
    const isCategoryOnChange = (e)=>{
        ...
        setCurrentPage(1);
        setSearch("")
        navigate('/data/brand?page=1&category='+e.target.value.split('_')[1]);
    }
    const edit =(id)=>{
        navigate(`/data/brand/edit/${id}`,{state:{page:currentPage,category:isCategory}})
    }

    const remove =(id)=>{
        axios.delete(`${process.env.REACT_APP_URL_API_DATA}/brand/delete/${id}`,{withCredentials:true}).then((res)=>{
            toggle(prev=>!prev);
        })
    }
    ...
    return ( <>
        ...
        <div className='d-flex justify-content-between align-items-center  mx-4'>
            <span>
            <select name='is_category' onChange={isCategoryOnChange} className='btn btn-primary m-4' value={Object.keys(isCategory)[0]}>
