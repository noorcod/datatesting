...
const AddResolution = () => {
    ...
    const submitForm =(e)=>{
        ...
        if(label.trim().length > 0){
            setSubmiting(true);
            axios.post(process.env.REACT_APP_URL_API_DATA+'/resolution/add',{rs_label:label},{ withCredentials: true}).then((res)=>{
                setSubmited(true);
                setTimeout(()=>{
                    setSubmiting(false);
                    setSubmited(false);
    ...
    return ( <>
        ...
        <div className='d-flex justify-content-center mx-4 mb-2'>
             <h2 className='text-secondary'>Add Resolution</h2>
        </div>
