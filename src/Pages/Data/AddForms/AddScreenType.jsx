...
const AddScreenType = () => {
    ...
    const submitForm =(e)=>{
        ...
        if(label.trim().length > 0){
            setSubmiting(true);
            axios.post(process.env.REACT_APP_URL_API_DATA+'/screen-type/add',{st_label:label},{ withCredentials: true}).then((res)=>{
                setSubmited(true);
                setTimeout(()=>{
                    setSubmiting(false);
                    setSubmited(false);
    ...
    <div className='d-flex justify-content-center mx-4 mb-2'>
             <h2 className='text-secondary'>Add Screen Type</h2>
        </div>
