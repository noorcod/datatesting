...
const LaptopType = () => {
    ...
    const changePage = (page)=>{
        setCurrentPage(page);
        navigate('/data/laptop-type?page='+page);
    }

    useEffect(()=>{
        const pageTimeout = setTimeout(()=>{
            axios.post(process.env.REACT_APP_URL_API_DATA+'/laptop-type/no-of-pages?size='+sizeOfPages,{search:search}).then((res)=>{
                if(res.data.no_of_pages === 0){
                    setNoRecord(true);
                    if(searchParams.get('page')){
                        setNoOfPages(0)
                        navigate('/data/laptop-type?page=0');
                    }
                }else{
                    setNoRecord(false);
                    ...
                    if(currentPage>res.data.no_of_pages){
                        ...
                        navigate('/data/laptop-type?page='+res.data.no_of_pages);
                    }
                    if(searchParams.get('page')=== '0'){
                        setCurrentPage(1)
                        navigate('/data/laptop-type?page=1');
                    }
                }
    ...
    useEffect(()=>{
        const dataTimeout = setTimeout(()=>{
            if(!noRecord){
                axios.post(process.env.REACT_APP_URL_API_DATA+'/laptop-type/get?page='+currentPage+'&size='+sizeOfPages,{search:search}).then((res)=>{
                    setData(res.data)
                })
            }
        },DELAY_TIME);
    ...
    const edit =(id)=>{
        navigate(`/data/laptop-type/edit/${id}`,{state:{page:currentPage}});
    }

    const remove =(id)=>{
        axios.delete(`${process.env.REACT_APP_URL_API_DATA}/laptop-type/delete/${id}`,{ withCredentials: true}).then((res)=>{
            toggle(prev=>!prev);
        })
    }
