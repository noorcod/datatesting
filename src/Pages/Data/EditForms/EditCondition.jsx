...
const EditCondition = () => {
    ...
    useEffect(()=>{
        axios.get(process.env.REACT_APP_URL_API_DATA+'/condition/get/'+id).then((res)=>{
    ...
    const submitForm =(e)=>{
        ...
        if(label.trim().length > 0){
            ...
            axios.patch(process.env.REACT_APP_URL_API_DATA+'/condition/update',obj,{withCredentials:true}).then((res)=>{
                setSubmited(true);
                setTimeout(()=>{
                    setSubmiting(false);
                    setSubmited(false);
                    navigate('/data/condition?page='+location.state.page);
                },2000)
            }).catch(()=>{
                setFailed(true);
                setTimeout(()=>{
    ...
    return ( <>
        ...
        <div className='d-flex justify-content-center mx-4 mb-2'>
             <h2 className='text-secondary'>Edit Condition</h2>
        </div>
        <Form noValidate validated={validated} onSubmit={submitForm}>
            <Form.Group className='mb-3' controlId='label'>
                <Form.Label className='asterik-label'>Label:</Form.Label>
                <Form.Control type='text' placeholder='Enter Condition' value={label} onChange={labelHandle} required/>
