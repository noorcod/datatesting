...
const EditDesktopType = () => {
    ...
    useEffect(()=>{
        axios.get(process.env.REACT_APP_URL_API_DATA+'/desktop-type/get/'+id).then((res)=>{
    ...
    const submitForm =(e)=>{
        ...
        if(label.trim().length > 0){
            ...
            axios.patch(process.env.REACT_APP_URL_API_DATA+'/desktop-type/update',obj,{ withCredentials: true}).then((res)=>{
                setSubmited(true);
                setTimeout(()=>{
                    setSubmiting(false);
                    setSubmited(false);
    ...
    return ( <>
        ...
        <div className='d-flex justify-content-center mx-4 mb-2'>
             <h2 className='text-secondary'>Edit Desktop Type</h2>
        </div>
...
export default EditDesktopType;