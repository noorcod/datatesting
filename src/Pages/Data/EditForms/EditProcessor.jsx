...
const EditProcessor = () => {
    ...
    useEffect(()=>{
        axios.get(process.env.REACT_APP_URL_API_DATA+'/processor/get/'+id).then((res)=>{
    ...
    const submitForm =(e)=>{
        ...
        if(checked){
            if(label.trim().length > 0){
                ...
                axios.patch(process.env.REACT_APP_URL_API_DATA+'/processor/update',
                obj,{ withCredentials: true}
                    ...
                    setTimeout(()=>{
                        setSubmiting(false);
                        setSubmited(false);
                        navigate('/data/processor?page='+location.state.page+'&category='+Object.keys(location.state.category)[0].split('_')[1]);
                    },2000)
                }).catch(()=>{
                    setFailed(true);
                    setTimeout(()=>{
    ...
    return ( <>
        ...
        <div className='d-flex justify-content-center mx-4 mb-2'>
             <h2 className='text-secondary'>Edit Processor</h2>
        </div>
        <Form noValidate validated={validated} onSubmit={submitForm}>
            <Form.Group className='mb-3' controlId='label'>
                <Form.Label className='asterik-label'>Label:</Form.Label>
                <Form.Control type='text' placeholder='Enter Processor' value={label} onChange={labelHandle} required/>
