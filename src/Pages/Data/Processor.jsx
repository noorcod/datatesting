...
const Processor = () => {
    ...
    const changePage = (page)=>{
        setCurrentPage(page);
        navigate('/data/processor?page='+page+"&category="+Object.keys(isCategory)[0].split('_')[1]);
    }

    useEffect(()=>{
        const pageTimeout = setTimeout(()=>{
            axios.post(process.env.REACT_APP_URL_API_DATA+'/processor/no-of-pages?size='+sizeOfPages,{...isCategory,search:search}).then((res)=>{
                if(res.data.no_of_pages === 0){
                    setNoRecord(true);
                    if(searchParams.get('page')){   
                        setNoOfPages(0)
                        navigate('/data/processor?page=0&category='+Object.keys(isCategory)[0].split('_')[1]);
                    }
                }else{
                    setNoRecord(false);
                    ...
                    if(currentPage>res.data.no_of_pages){
                        ...
                        navigate('/data/processor?page='+res.data.no_of_pages+"&category="+Object.keys(isCategory)[0].split('_')[1]);
                    }
                    if(searchParams.get('page')=== '0'){
                        setCurrentPage(1)
                        navigate('/data/processor?page=1&category='+Object.keys(isCategory)[0].split('_')[1]);
                    }
                }
            })
        },DELAY_TIME);
    ...
    },[sizeOfPages,data,currentPage,isCategory,searchParams,navigate,search]);

    
    useEffect(()=>{
        const dataTimeout = setTimeout(()=>{
            if(!noRecord){
                axios.post(process.env.REACT_APP_URL_API_DATA+'/processor/get?page='+currentPage+'&size='+sizeOfPages,{...isCategory,search:search}).then((res)=>{
                    setData(res.data)
                })
            }
        },DELAY_TIME);
        return ()=>{
            ...
            if(currentSearch.current !== search){
                setCurrentPage(1);
                navigate('/data/processor?page=1');
            }
        }
    ...
    const isCategoryOnChange = (e)=>{
        ...
        setIsCategory(temp);
        setSearch("")
        navigate('/data/processor?page='+currentPage+"&category="+e.target.value.split('_')[1]);
    
    }

    const edit =(id)=>{
        navigate(`/data/processor/edit/${id}`,{state:{page:currentPage,category:isCategory}});
    }

    const remove =(id)=>{
        axios.delete(`${process.env.REACT_APP_URL_API_DATA}/processor/delete/${id}`,{ withCredentials: true}).then((res)=>{
            toggle(prev=>!prev);
        })
    }
    ...
    return ( <>
        <div className='d-flex justify-content-center mx-4 mb-2'>
             <h2 className='text-secondary'>Processor</h2>
        </div>
