...
const Tabs = () => {
    ...
    const changePage = (page)=>{
        setCurrentPage(page);
        navigate('/tabs?page='+page);
    }

    useEffect(()=>{
        const pageTimeout = setTimeout(()=>{
            axios.get(process.env.REACT_APP_URL_API_DATA+'/model/tabs/no-of-pages?size='+sizeOfPages+'&search='+search).then((res)=>{
                if(res.data.no_of_pages === 0){
                    setNoRecord(true);
                    if(searchParams.get('page')){   
                        setNoOfPages(0)
                        navigate('/tabs?page=0');
                    }
                }else{
                    setNoRecord(false);
                    ...
                    if(currentPage>res.data.no_of_pages){
                        ...
                        navigate('/tabs?page='+res.data.no_of_pages);
                    }
                    if(searchParams.get('page')=== '0'){
                        setCurrentPage(1)
                        navigate('/tabs?page=1');
                    }
                }
            })
        },DELAY_TIME)
    ...
    useEffect(()=>{
        const dataTimeout = setTimeout(()=>{
            if(!noRecord){
                axios.post(process.env.REACT_APP_URL_API_DATA+'/model/tabs?page='+currentPage+'&size='+sizeOfPages,{search:search}).then((res)=>{
                    setData(res.data)
                })
            }
        },DELAY_TIME)
    ...
    const edit = (id) => {
        navigate('/tabs/edit/'+id,{state:{page:currentPage}})
    }
