...
const LEDs = () => {
  ...
  useEffect(() => {
    const pageTimeout = setTimeout(() => {
      axios
        .get(
          process.env.REACT_APP_URL_API_DATA +
            "/model/leds/no-of-pages?size=" +
            sizeOfPages +
            "&search=" +
            search
        )
        .then((res) => {
          if (res.data.no_of_pages === 0) {
            ...
            if (currentPage > res.data.no_of_pages) {
              ...
              navigate("/leds?page=" + res.data.no_of_pages);
            }
            if (searchParams.get("page") === "0") {
              setCurrentPage(1);
              navigate("/leds?page=1");
            }
          }
        });
    }, DELAY_TIME);
  ...
  const edit = (id) => {
    navigate("/leds/edit/" + id, { state: { page: currentPage } });
  };

  const remove = (id) => {
    axios
      .delete(`${process.env.REACT_APP_URL_API_DATA}/model/delete/${id}`,{withCredentials:true})
      .then((res) => {
        toggle((prev) => !prev);
      });
  };
  ...
  return (
    <>
      <div className="d-flex justify-content-center mx-4 mb-2">
        <h2 className="text-secondary">LEDs</h2>
      </div>
      <div className="d-flex justify-content-between align-items-center  mx-4">
        ...
        />
        <Link to="/leds/add" state={{ page: noOfPages +1}}>
          <button>Add LED</button>
        </Link>
      </div>
      {noRecord ? (
