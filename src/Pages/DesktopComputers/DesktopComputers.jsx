...
const DesktopComputers = () => {
    ...
    const changePage = (page)=>{
        setCurrentPage(page);
        navigate('/desktop-computers?page='+page);
    }

    useEffect(()=>{
        const pageTimeout = setTimeout(()=>{
            axios.get(process.env.REACT_APP_URL_API_DATA+'/model/desktop-computers/no-of-pages?size='+sizeOfPages+'&search='+search).then((res)=>{
                if(res.data.no_of_pages === 0){
                    setNoRecord(true);
                    if(searchParams.get('page')){   
                        setNoOfPages(0)
                        navigate('/desktop-computers?page=0');
                    }
                }else{
                    setNoRecord(false);
                    ...
                    if(currentPage>res.data.no_of_pages){
                        ...
                        navigate('/desktop-computers?page='+res.data.no_of_pages);
                    }
                    if(searchParams.get('page')=== '0'){
                        setCurrentPage(1)
                        navigate('/desktop-computers?page=1');
                    }
                }
            })
        },DELAY_TIME)
    ...
    useEffect(()=>{
        const dataTimeout = setTimeout(()=>{
            if(!noRecord){
                axios.post(process.env.REACT_APP_URL_API_DATA+'/model/desktop-computers?page='+currentPage+'&size='+sizeOfPages,{search:search}).then((res)=>{
                    setData(res.data)
                })
            }
        },DELAY_TIME)
    ...
    const edit = (id) => {
        navigate('/desktop-computers/edit/'+id,{state:{page:currentPage}});
    }
    ...
    return ( <>
        <div className='d-flex justify-content-center mx-4 mb-2'>
             <h2 className='text-secondary'>Desktop Computers</h2>
        </div>
        <div className='d-flex justify-content-between align-items-center  mx-4'>
            <input type='search' placeholder='Search' className='px-3 rounded' value={search} onChange={handleSearchInput} />
            <Link to='/desktop-computers/add' state={{page:noOfPages+1}}><button>Add Desktop Computer</button></Link>
        </div>
        {noRecord ? (
